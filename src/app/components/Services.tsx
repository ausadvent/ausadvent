...
export default function Services() {
  ...
  return (
    <div className='page sm:mx-auto py-[2rem] md:py-[4.5rem] lg:py-[5rem]'>
        <h2 className='cormorant text-[1.875rem] md:text-[3rem] font-bold md:text-center'>OUR SERVICES</h2>
        <p className='mt-[3rem] md:mt-[4.5rem] lg:mt-[5rem] text-[1rem] md:text-[1.25rem] leading-[1.5rem] md:leading-[1.75rem] md:text-center'>Our <strong>Supported Accommodation</strong> service is designed to empower individuals with tailored solutions for independent living.</p>

        {/* Services cards container */}
        <div className='mt-[3rem] md:mt-[4.5rem] lg:mt-[5rem] flex flex-col gap-[3rem] md:gap-[8rem]'>
          {services?.slice().reverse().map((service:any, index:any) => (
            <div key={service?.fields.serviceUrl} className={`flex flex-col md:flex-row gap-[2rem] ${index % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
              ...
              <div className='relative'>
                <div className='md:w-[19rem] lg:w-[23rem]'>
                  <Image 
                    src={`http:${service.fields.serviceMainImage.fields.file.url}`}
                    className='w-[22.37rem] sm:w-[31.25rem] md:w-[19rem] lg:w-[23rem] h-[18rem] rounded-tr-3xl rounded-bl-3xl object-cover'
