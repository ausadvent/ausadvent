...
export default async function Values() {
    ...
    <div className='page sm:px-[2rem] sm:mx-auto py-[3rem] md:py-[4.5rem] lg:py-[5rem]'>
        ...
        {values.slice().reverse().map((value:any, index:any) => (
            <div key={index} className='py-[3rem] px-[1rem] bg-[#FFEDD5] rounded-tr-xl rounded-bl-xl flex flex-col md:flex-row items-center gap-[0.5rem] sm:gap-[1rem]'>
                <Image 
                    src={value?.fields?.valueImage.fields.file.url} 
                    className='w-[6rem] h-[6rem] md:w-[16rem] md:h-[16rem]'
                    ...
                    // unoptimized
                />
                <div className='flex flex-col items-center md:items-start gap-[0.5rem] sm:gap-[1rem] '>
                    ...
                    <p className='leading-[1.5rem] text-center md:text-left'>{value?.fields?.valueDescription}</p>
