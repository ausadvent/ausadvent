...
export default async function Values() {
    ...
    return (
    <div className='page py-[3rem]'>
        <h2 className='cormorant text-[1.875rem] font-bold'>OUR CORE VALUES</h2>
        ...
        {values.slice().reverse().map((value:any, index:any) => (
            <div key={index} className='py-[3rem] px-[1rem] bg-[#FFEDD5] rounded-tr-xl rounded-bl-xl flex flex-col items-center gap-[0.5rem]'>
                <Image 
                ...
                <p className='leading-[1.5rem]'>{value?.fields?.valueDescription}</p>
            </div>
        ))}
        </div>
    </div>
